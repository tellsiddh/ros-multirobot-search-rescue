...
class Explorer:
    def __init__(self, ns):
        ...
        # Initialize variables to store odometry, frontiers, safespots, and move_base status information
    ...
    def odometry_callback(self, msg):
        ...
        if self.frontiers is not None:
            self.navigate_to_closest_frontier()
    ...
    def navigate_to_closest_frontier(self):
        ...
        if not valid_frontier_indices:
            ...
            if self.current_goal is None or np.min(goal_distances) > 1 or self.rejected == True or self.reached_goal == True:
                # Publish a goal to move_base_simple
                goal_msg = PoseStamped()
                goal_msg.header.stamp = rospy.Time.now()
                goal_msg.header.frame_id = 'map'
